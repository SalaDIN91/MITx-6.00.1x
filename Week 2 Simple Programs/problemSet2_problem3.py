# -*- coding: utf-8 -*-
"""
Created on Mon Nov  6 21:50:24 2017
...
is a multiple of $0.01). Does your code still work? It should, but you may notice 
that your code runs more slowly, especially in cases with very large balances and 
...
the entire balance at the end of the year. What we ultimately pay must be greater 
than what we would've paid in monthly installments, because the interest was 
compounded on the balance we didn't pay off each month. So a good upper bound 
for the monthly payment would be one-twelfth of the balance, after having its 
...
to the cent (no more multiples of $10) such that we can pay off the debt within a 
year. Try it out with large inputs, and notice how fast it is (try the same large 
inputs in your solution to Problem 2 to compare!). Produce the same return value 
as you did in Problem 2.

"""

#code
def minMonthlyPayment(balance, lowerMonthly, upperMonthly, midMonthly):
  N = 12
  bal = balance
  while N > 0:
    bal = bal - midMonthly
    bal = bal + monthlyInterestRate*bal
    N -= 1
  if abs(upperMonthly - lowerMonthly) < 0.02:
    return midMonthly
  if bal > 0:
    return minMonthlyPayment(balance, midMonthly, upperMonthly, (upperMonthly + midMonthly)/2)
  else:
    return minMonthlyPayment(balance, lowerMonthly, midMonthly, (lowerMonthly + midMonthly)/2)

#balance = 341239
#annualInterestRate = 0.2
monthlyInterestRate = annualInterestRate/12
lowerMonthly = balance/12
upperMonthly = balance*((1+monthlyInterestRate)**12)/12
midMonthly = (lowerMonthly+upperMonthly)/2
print("Lowest Payment:",round(minMonthlyPayment(balance, lowerMonthly, upperMonthly, midMonthly),2))